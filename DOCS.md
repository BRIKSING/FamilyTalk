...
## Data Models (Слой данных)

**Файлы:** `Models/User.swift`, `Models/Chat.swift`, `Models/Message.swift`,
`Models/CallLog.swift`, `Models/AuthResponse.swift`

### Назначение

Модуль **Data Models** — это набор `Codable`-структур, описывающих доменные
сущности FamilyTalk: пользователя, чат, сообщение, запись журнала звонков и
ответ авторизации. Это «общий язык» всего бэкенда: модели одновременно являются
и **DTO** (объектами, в которые декодируется JSON от REST API и Socket.IO), и
**доменными моделями**, которыми оперируют сервисы и ViewModels.

Ключевые принципы модуля:

- **Только данные, без сетевой логики.** Модели не знают ни о `NetworkService`,
  ни о `URLSession`. Они пассивны и не выполняют запросов — их лишь заполняют
  сервисы после декодирования ответа.
- **Значимые типы (structs).** Все модели — `struct`, что даёт value-семантику
  (копирование вместо ссылок) и хорошо ложится на Swift 6 concurrency: их можно
  безопасно передавать между акторами.
- **Вычислимые свойства для представления.** Часть форматирования (онлайн-статус,
  «удалено», длительность звонка) вынесена в вычисляемые свойства прямо на моделях,
  чтобы UI и ViewModels не дублировали эту логику. Это единственная «логика»
  внутри слоя данных, и она детерминированная и без побочных эффектов.

### Связи с другими модулями

Слой данных — фундамент, от которого зависят практически все остальные модули
бэкенда, а сам он не зависит ни от кого (кроме `Foundation`).

- **NetworkService → Models.** Дженерик-метод `request<T: Decodable>` декодирует
  тело ответа в конкретную модель (`[Chat]`, `Message`, `User` и т. д.). Именно
  здесь применяется кастомная `dateDecodingStrategy` (см. ниже про даты).
- **SocketService → Models.** Входящие realtime-события (`message:new`,
  `message:edited`, …) декодируются в те же модели (`Message` и производные
  payload-структуры) собственным `JSONDecoder` с идентичной стратегией дат.
- **AuthService → `AuthResponse`, `LoginRequest`, `User`.** Логин отправляет
  `LoginRequest` и получает `AuthResponse` (токен + профиль); `User` также
  кэшируется/восстанавливается при авто-логине.
- **ChatService → `Chat`, `ChatMember`, `Message`, `MessageReplyPreview`.**
  Оперирует чатами и сообщениями; наружу отдаёт уже развёрнутые доменные модели.
- **ContactsService → `User`.** Контакты, поиск и профиль — это списки/одиночные
  `User`.
- **CallService → `CallLog`, `User`.** Журнал звонков; в каждой записи опционально
  вложены профили `initiator`/`target`.
- **ViewModels (фронтенд) → Models.** Презентационный слой хранит и отображает
  эти же модели. С точки зрения бэкенда важно: наружу всегда отдаются доменные
  модели, а не сырой JSON и не приватные обёртки запросов/ответов.

Схематично:

```
              ┌────────────────────────────┐
   JSON ─────►│ NetworkService / Socket    │──► decode ──►┐
              └────────────────────────────┘              │
                                                          ▼
   ┌───────────────────────────────────────────────────────────┐
   │  Data Models:  User · Chat · Message · CallLog · Auth...   │
   └───────────────────────────────────────────────────────────┘
                                                          │
   AuthService · ChatService · ContactsService ···········┘
   CallService · SocketService · ViewModels  (потребители)
```
...
Профиль пользователя. `Identifiable, Codable, Equatable`.

| Поле | Тип | Смысл |
|------|-----|-------|
| `id` | `String` | Идентификатор пользователя |
| `displayName` | `String` | Отображаемое имя |
| `phone` | `String?` | Телефон (может отсутствовать в публичном профиле) |
| `username` | `String?` | Юзернейм для поиска |
| `avatarUrl` | `String?` | Ссылка на аватар |
...
- `isOnline: Bool` — `true`, если `lastSeen` был менее **30 секунд** назад.
  Порог онлайна зашит здесь, а не приходит с сервера.
- `lastSeenText: String` — человекочитаемая строка активности: `«только что»`,
  `«N мин назад»`, `«N ч назад»`, затем дата в формате `dd.MM.yyyy`; при
  отсутствии `lastSeen` — `«Давно»`.

#### `Chat` и `ChatMember` (`Models/Chat.swift`)

Чат и его участник. `Chat` — `Identifiable, Codable, Hashable`.

`ChatType`: `.direct` (`"DIRECT"`) или `.group` (`"GROUP"`).

**`ChatMember`** (`Codable, Identifiable`, где `id == userId`):

| Поле | Тип | Смысл |
|------|-----|-------|
| `chatId` | `String` | К какому чату относится участник |
| `userId` | `String` | Идентификатор пользователя |
| `joinedAt` | `Date` | Когда присоединился |
| `user` | `User?` | Опционально вложенный профиль |
...
| `name` | `String?` | Имя (для групп; у личных обычно `nil`) |
| `avatarUrl` | `String?` | Аватар чата |
| `createdAt` | `Date` | Момент создания |
| `members` | `[ChatMember]` | Участники |
| `lastMessage` | `Message?` | Последнее сообщение (превью в списке) |
| `unreadCount` | `Int` | Число непрочитанных |

Хелперы: `displayName(currentUserId:)` (для личного чата берёт имя собеседника,
для группы — `name`) и `otherUser(currentUserId:)`. **Важно:** равенство (`==`)
и `hash(into:)` считаются **только по `id`** — два объекта `Chat` с одинаковым
`id`, но разным `unreadCount`, считаются равными. Это удобно для `NavigationStack`
и `Set`, но означает, что нельзя полагаться на `==`, чтобы заметить изменение
непрочитанных — сравнивайте поля явно.

#### `Message`, `MessageReplyPreview`, `MessageType` (`Models/Message.swift`)

Сообщение чата. `Message` — `Identifiable, Codable, Equatable`.

`MessageType`: `.text` (`"TEXT"`) или `.system` (`"SYSTEM"`).

**`Message`**:

| Поле | Тип | Смысл |
|------|-----|-------|
| `id` | `String` (var) | Идентификатор; **изменяемый**, чтобы заменить оптимистичный локальный id после ack сервера |
| `chatId` | `String` | К какому чату относится |
| `senderId` | `String` | Автор |
| `type` | `MessageType` | `.text` / `.system` |
| `content` | `String?` | Текст (может быть `nil`, например у удалённого) |
| `replyToId` | `String?` | id сообщения, на которое отвечают |
| `editedAt` | `Date?` | Не `nil`, если сообщение редактировали |
| `deletedAt` | `Date?` | Не `nil`, если удалено |
| `createdAt` | `Date` | Момент отправки |
| `sender` | `User?` | Вложенный профиль автора |
| `replyTo` | `MessageReplyPreview?` | Превью цитируемого сообщения |

Вычисляемые: `isDeleted` (`deletedAt != nil`), `isEdited` (`editedAt != nil`)
и `displayContent` — возвращает `«Сообщение удалено»` для удалённых и `content ?? ""`
в остальных случаях (можно показывать в UI без доп. проверок).

**`MessageReplyPreview`** — облегчённая версия сообщения, вложенная в `Message.replyTo`
для отрисовки цитаты без загрузки полного оригинала: `id`, `senderId`, `content?`,
`type`, `deletedAt?` и вычисляемое `isDeleted`.

#### `CallLog`, `CallType`, `CallStatus` (`Models/CallLog.swift`)

Запись журнала звонков. `Identifiable, Codable`.
...
| `initiatorId` | `String` | Кто инициировал звонок |
| `targetId` | `String` | Кому звонили |
| `type` | `CallType` | Голос / видео |
| `status` | `CallStatus?` | Итог звонка (может отсутствовать для незавершённых) |
| `startedAt` | `Date?` | Начало разговора |
| `endedAt` | `Date?` | Конец разговора |
| `createdAt` | `Date` | Момент создания записи |
| `initiator` | `User?` | Вложенный профиль инициатора |
| `target` | `User?` | Вложенный профиль собеседника |

Вычисляемые: `duration: TimeInterval?` (разница `endedAt − startedAt`, `nil` если
хотя бы одна из дат отсутствует) и `durationText` — формат `«M:SS»`, либо `«—»`,
если длительность неизвестна.

#### `AuthResponse` и `LoginRequest` (`Models/AuthResponse.swift`)

Транспортные модели авторизации.

- **`LoginRequest`** — тело `POST /auth/login`: `phone: String`, `displayName: String`.
- **`AuthResponse`** — ответ `POST /auth/login`: `accessToken: String` и `user: User`.

### Как использовать

Модели самодостаточны — их не нужно конструировать вручную в типичном сценарии:
сервисы возвращают уже готовые объекты. Достаточно объявить переменную нужного типа.

```swift
// Получение и использование готовых доменных моделей
let chats: [Chat] = try await ChatService.shared.fetchChats()
for chat in chats {
    let title = chat.displayName(currentUserId: myId)  // хелпер модели
    let preview = chat.lastMessage?.displayContent ?? "" // безопасный текст
    print("\(title): \(preview) [\(chat.unreadCount)]")
}
```

```swift
// Отображение статуса пользователя без дублирования логики в UI
let user: User = contact
Text(user.displayName)
Text(user.isOnline ? "в сети" : user.lastSeenText)
```

```swift
// Ручное создание модели нужно в основном для оптимистичных сообщений и тестов.
// createdAt намеренно ставится клиентом, id позже заменится серверным.
...
- **Формат дат.** Все `Date`-поля декодируются кастомной стратегией
  (`NetworkService` и `SocketService` настроены одинаково): сначала пробуется
  ISO 8601 **с** дробными секундами (`.withFractionalSeconds`), затем **без**
  них; если ни один формат не подошёл — бросается `DecodingError`. Значит, сервер
  обязан присылать даты в ISO 8601; Unix-timestamp'ы или иные форматы не пройдут.
- **Строковые enum'ы.** `ChatType`, `MessageType`, `CallType`, `CallStatus` —
  `String`-backed enum'ы с **заглавными** raw-значениями (`"DIRECT"`, `"TEXT"`,
  `"VIDEO"`, `"MISSED"`). JSON от сервера должен использовать ровно эти строки,
  иначе декодирование упадёт. Регистр важен.
- **Опциональность = контракт с сервером.** Многие поля опциональны (`phone`,
  `avatarUrl`, `status`, `startedAt`, вложенные `user`/`sender`). Это осознанный
  выбор: сервер может не присылать вложенные профили или незавершённые данные.
  Отсутствие поля в JSON декодируется в `nil`, а не в ошибку.
- **`Message.id` изменяемый (`var`) намеренно.** Это обеспечивает паттерн
  оптимистичной отправки: клиент создаёт сообщение с временным id, показывает его
  сразу, а после ack сервера заменяет `id` на настоящий — не пересоздавая объект.
- **Равенство `Chat` только по `id`.** См. предупреждение выше: изменения полей
  вроде `unreadCount` не отражаются в `==`/`hash`. `User`, `Message`, `CallLog`
  и остальные используют синтезированное покомпонентное равенство/декодирование.
- **Логика представления живёт на моделях.** Пороги и форматы (30 сек для
  онлайна, шаблоны «N мин назад», `«M:SS»`) захардкожены в вычисляемых свойствах.
  При изменении требований к отображению правится именно модель — это единая точка.

### Точки расширения

- **Синхронизация enum'ов с сервером.** При добавлении новых типов сообщений/звонков
  (например, `IMAGE`, `AUDIO`, `GROUP_CALL`) нужно расширить соответствующий enum.
  Чтобы неизвестное значение не роняло всё декодирование, стоит рассмотреть
  fallback-кейс (`case unknown`) с ручной реализацией `init(from:)`.
- **Отделение DTO от доменных моделей.** Пока это одни и те же структуры. Если
  контракт API начнёт сильно расходиться с потребностями UI, слой можно разбить на
  сетевые DTO и доменные модели с маппингом в сервисах — публичный API сервисов
  при этом не изменится.
- **Единый декодер.** Стратегия дат сейчас продублирована в `NetworkService` и
  `SocketService`. Её логично вынести в общее расширение `JSONDecoder`
  (например, `JSONDecoder.familyTalk`) рядом со слоем данных, чтобы гарантировать
  одинаковый разбор во всех точках входа.
- **Локализация строк представления.** `lastSeenText`, `«Сообщение удалено»`,
  `durationText` захардкожены по-русски. При выходе на мультиязычность их стоит
  перевести на `LocalizedStringKey`/`String(localized:)`.
