...
| ✅ | **CallService** | `Services/CallService.swift` | История звонков (REST); сигналинг звонков — в SocketService |
...
**Зависимости:** `NetworkService.shared` (транспорт) · модель `CallLog` (`Models/CallLog.swift`)

### Назначение

`CallService` — это **доменный REST-сервис истории звонков**. Его единственная
задача — отдавать **журнал завершённых звонков** (входящих и исходящих, голосовых и
видео) постранично, для отображения на вкладке «Звонки».

Важно понимать границу ответственности модуля: **`CallService` не устанавливает и не
ведёт звонки**. Он работает только с уже произошедшими фактами звонков — это «журнал»,
а не «телефон». Всё живое взаимодействие звонка — приглашение (offer), ответ (answer),
обмен ICE-кандидатами и завершение (hangup/decline) — идёт **не через REST**, а через
реалтайм-слой `SocketService` поверх Socket.IO (см. раздел «SocketService → Call
Emit / Call events»). `CallService` подключается уже после: когда звонок закончился и
сервер записал его в историю.

Соответственно, у сервиса **один-единственный метод** — `fetchHistory`,
реализующий `GET /calls/history` с **курсорной пагинацией**.

### Связи с другими модулями

```
   CallViewModel ───────────────► CallService.shared
     (фронтенд)                      │  fetchHistory(cursor:limit:)
                                     │
                                     ▼
                              NetworkService.shared ──► URLSession ──► REST API
                                     ▲                                   GET /calls/history
                                     │  токен (Bearer) уже проставлен AuthService
                                     │
   Модель-контракт:  CallHistoryPage { items: [CallLog], nextCursor: String? }

   ─────────────────────────────────────────────────────────────────────────
   Живой звонок (offer/answer/ICE/hangup) идёт МИМО CallService:
   CallViewModel ──► SocketService.shared  (call:offer / call:answer / … )
```

- **Транспорт — `NetworkService`.** Сервис хранит
  `private let network = NetworkService.shared` и делает единственный вызов через
  `network.request(...)`. Авторизация, таймауты, проверка HTTP-статуса, разбор дат
  (`startedAt` / `endedAt` / `createdAt`) и маппинг ошибок в `NetworkError` —
  всё это внутри `NetworkService`; сам `CallService` про это ничего не знает.
- **Авторизация — косвенно через `AuthService`.** Запрос идёт с `requiresAuth: true`
  (значение по умолчанию в `NetworkService`): токен подставляет `AuthService`, при
  `401` централизованно происходит `logout()`. `CallService` в авторизацию не
  вмешивается.
- **Реалтайм-сигналинг — `SocketService`, а не `CallService`.** Это ключевая связь
  «по разделению обязанностей»: установление и ведение звонка целиком лежит на
  `SocketService` (`sendOffer`, `sendAnswer`, `sendIceCandidate`, `sendHangup`,
  `sendDecline` и входящие события `call:incoming` / `call:answered` /
  `call:ice-candidate` / `call:hangup` / `call:declined`). Когда звонок завершается,
  сервер сохраняет `CallLog`, и уже он попадает в выдачу `fetchHistory`. Два модуля
  дополняют друг друга: `SocketService` — «во время звонка», `CallService` — «после».
- **Модель-контракт — `CallLog`** (`Models/CallLog.swift`). `fetchHistory` возвращает
  страницу `CallHistoryPage`, состоящую из `[CallLog]`. `CallLog` содержит вложенные
  `initiator` / `target` типа `User` — той же доменной модели пользователя, что и в
  `AuthService`, `ContactsService`, `ChatService`.
- **Потребитель (фронтенд).** `CallViewModel` вызывает `fetchHistory` и наблюдается
  экраном `CallView`. Это презентационный слой и в данном документе подробно не
  рассматривается.

### Модель данных

Сервис возвращает страницу истории **`CallHistoryPage`** — тонкую обёртку для
курсорной пагинации:

| Поле | Тип | Смысл |
|------|-----|-------|
| `items` | `[CallLog]` | Звонки текущей страницы (обычно от новых к старым) |
| `nextCursor` | `String?` | Курсор следующей страницы; `nil` — история исчерпана |

Элемент истории — **`CallLog`** (`Identifiable, Codable`):
...
| `startedAt` | `Date?` | Момент начала разговора (после ответа) |
| `endedAt` | `Date?` | Момент завершения |
| `createdAt` | `Date` | Момент создания записи (используется для сортировки/показа) |
| `initiator` | `User?` | Развёрнутый инициатор (если сервер приложил) |
| `target` | `User?` | Развёрнутый адресат (если сервер приложил) |

Два вычисляемых свойства `CallLog`, полезных для UI:

- **`duration`** — `TimeInterval?`; длительность разговора (`endedAt − startedAt`),
  либо `nil`, если звонок не состоялся (пропущенный/отклонённый).
- **`durationText`** — форматированная строка `"m:ss"` (например `"3:07"`), либо
...
// GET /calls/history — постраничная история звонков (курсорная пагинация).
// cursor: nil — первая страница; limit клампится сверху до 100.
func fetchHistory(cursor: String? = nil, limit: Int = 20) async throws -> CallHistoryPage
```

Тип ответа `CallHistoryPage` — публичный (в отличие от приватных обёрток некоторых
других сервисов), поскольку через него наружу отдаётся курсор для подгрузки
следующей страницы.

### Как использовать

**1. Загрузить первую страницу истории (например, при открытии вкладки «Звонки»):**
...
**2. Подгрузить следующую страницу (бесконечная прокрутка):**
...
repeat {
    let page = try await CallService.shared.fetchHistory(cursor: cursor, limit: 30)
    all.append(contentsOf: page.items)
    cursor = page.nextCursor
} while cursor != nil          // nextCursor == nil → история исчерпана
```

**3. Показать элемент истории:**

```swift
let call = page.items[0]
let who = call.type == .video ? "Видеозвонок" : "Звонок"
let dur = call.durationText                 // "3:07" или "—"
let missed = call.status == .missed         // пометить пропущенные
```

### Обработка ошибок

Собственного типа ошибок у `CallService` нет — наружу пробрасываются те же
`NetworkError`, что и из `NetworkService` (см. раздел «NetworkService → Обработка
ошибок»). Шаблон вызова:
...
do {
    ...
    print(error.errorDescription ?? "Не удалось загрузить историю звонков")
}
```

### Детали реализации, важные для интеграции

- **Синглтон без состояния.** `CallService.shared`, `init` приватный. Сервис не
  хранит кэшей и изменяемого состояния — чистый «фасад» над одним REST-эндпоинтом,
  безопасен для вызова из любого места.
- **`limit` клампится на клиенте.** В запрос уходит `min(limit, 100)` — даже если
  передать большее значение, сервер получит не больше 100. Пагинацию всегда стройте
  на `nextCursor`, а не на увеличении `limit`.
- **Курсор непрозрачен.** `nextCursor` — строка, формат которой определяет сервер;
  клиент не должен её парсить или конструировать. Просто передавайте полученное
  значение обратно в следующий вызов; `nil` означает конец истории.
- **Сигналинг — не здесь.** Если нужно **начать** звонок, отвечать на входящий или
  завершить текущий — это `SocketService`, а не `CallService`. `fetchHistory`
  отражает только уже завершённые звонки и не имеет побочных эффектов на активные
  сессии.
- **Опциональные поля — норма.** `status`, `startedAt`, `endedAt`, `initiator`,
  `target` объявлены опциональными: у пропущенного звонка нет `startedAt`/`endedAt`,
  а вложенные `User` сервер может не прикладывать. Всегда обрабатывайте `nil`
  (для этого и существуют `duration`/`durationText`).

### Точки расширения

- **Фильтрация истории.** Естественное развитие `fetchHistory` — параметры фильтра
  (только пропущенные, только видео, по конкретному собеседнику) через дополнительные
  query-параметры, по аналогии с тем, как устроены остальные доменные сервисы.
- **Удаление/очистка истории.** Логичное соседнее действие — `DELETE /calls/history`
  или удаление отдельной записи; добавляется рядом, не меняя `fetchHistory`.
- **Инициация звонка через фасад.** При желании иметь единую «точку входа» в звонки
  можно добавить в `CallService` методы-обёртки, делегирующие в `SocketService`
  (`startCall`, `answer`, `hangup`), сохранив реалтайм-механику внутри сокета.
- **Кэширование.** Модуль намеренно без кэша. Первую страницу истории при
  необходимости офлайн-доступа можно кэшировать внутри сервиса, не меняя публичный API.
