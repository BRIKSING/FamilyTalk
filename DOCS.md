...
**Зависимости:** `NetworkService`, `KeychainService`, `SocketService`, `NotificationCenter`, `UserDefaults`

### Назначение

`AuthService` — это **центр авторизации и владелец сессии** приложения. Он —
единственный источник правды о том, **вошёл ли пользователь** и **кто он такой**,
и координирует все побочные эффекты входа/выхода между остальными бэкенд-модулями.
Модуль решает четыре задачи:

1. **Логин.** Отправляет учётные данные на `POST /auth/login`, получает токен и
   профиль, сохраняет сессию.
2. **Хранение сессии.** Кладёт access-токен в Keychain (через `KeychainService`),
   а профиль пользователя — в `UserDefaults`, чтобы переживать перезапуск приложения.
3. **Восстановление входа.** При старте приложения (`init`) поднимает сохранённую
   сессию без повторного логина.
4. **Логаут и реакция на разавторизацию.** Централизованно чистит токен, профиль,
   рвёт сокет и сбрасывает состояние — как по явному выходу, так и автоматически
   при HTTP `401`.

Два наблюдаемых (`@Observable`) свойства — `currentUser` и `isAuthenticated` —
это **публичное состояние сессии**, на которое подписан UI (корневой экран
приложения решает по `isAuthenticated`, что показать — экран входа или основной
интерфейс).
...
```
   ...
   AuthViewModel ──login──► AuthService ──────request(...)──────► NetworkService ──► REST API
   (фронтенд)               │  │  │  │
      ▲                     │  │  │  └──save/load/delete "access_token"──► KeychainService ──► Keychain
      │ currentUser         │  │  └──────connect(token:) / disconnect()──► SocketService  ──► WebSocket
      │ isAuthenticated     │  └─────────encode/decode "current_user"────► UserDefaults
      └── @Observable ◄──────┘
                             ▲
        NotificationCenter   │  .networkUnauthorized (401)
        (от NetworkService) ─┘  → logout()
```

- **`NetworkService` (транспорт + токен).** `AuthService` вызывает `request(...)`
  для `login`/`fetchMe` и управляет токеном транспорта через `setAccessToken(_:)`
  (при входе/восстановлении) и `clearAccessToken()` (при выходе). Сам HTTP-слой
  токен на диск не пишет — это ответственность `AuthService`.
- **`KeychainService` (безопасное хранилище токена).** Токен живёт под ключом
  `"access_token"`. `AuthService` — единственный модуль, который его туда кладёт,
  читает и удаляет.
- **`UserDefaults` (кеш профиля).** Профиль `User` сериализуется в JSON под ключом
  `"current_user"` — это позволяет мгновенно восстановить `currentUser` при старте,
  не дожидаясь сети.
- **`SocketService` (жизненный цикл соединения).** Именно `AuthService` — владелец
  жизненного цикла реального времени: после успешного входа или восстановления
  сессии он вызывает `connect(token:)`, а при выходе — `disconnect()`. Токен
  передаётся аргументом (см. документацию `SocketService`).
- **`NotificationCenter` (реакция на `401`).** В `init` оформляется подписка на
  `Notification.Name.networkUnauthorized`, которую публикует `NetworkService` при
  ответе `401`. По этому событию выполняется `logout()`. Так протухший токен
  приводит к выходу **без прямой зависимости** `NetworkService → AuthService`.
- **Потребитель (фронтенд).** `AuthViewModel` вызывает `login`/`fetchMe`/`logout`
  и подписан на `currentUser`/`isAuthenticated`. ViewModel — это фронтенд и в этом
  документе не рассматривается.

### Публичное состояние и API

#### Наблюдаемое состояние сессии

```swift
private(set) var currentUser: User?        // текущий профиль (nil, если не авторизован)
private(set) var isAuthenticated: Bool     // true после успешного входа/восстановления
```

Оба свойства **только для чтения снаружи** (`private(set)`) — менять их может лишь
сам `AuthService`. UI на них подписывается через механизм `@Observable`.

#### Методы

```swift
// Вход: POST /auth/login. Сохраняет сессию и поднимает сокет. Возвращает профиль.
func login(phone: String, displayName: String) async throws -> User

// Обновить профиль с сервера: GET /auth/me. Пишет результат в currentUser.
func fetchMe() async throws -> User

// Выход: чистит Keychain, UserDefaults, токен транспорта, рвёт сокет, сбрасывает состояние.
@MainActor func logout()
```

> `login` и `fetchMe` — асинхронные и `throws`: ошибки приходят из
> `NetworkService` как `NetworkError` (см. соответствующий раздел). `saveSession`
> и `restoreSession` — приватные и в публичный API не входят.

### Как использовать

**1. Вход по номеру телефона и имени:**

```swift
do {
    let user = try await AuthService.shared.login(phone: phone, displayName: name)
    // сессия уже сохранена, сокет подключён — можно переходить в основной интерфейс
} catch let error as NetworkError {
    // показать error.errorDescription
}
```

**2. Реакция на состояние авторизации в корневом экране (SwiftUI):**

```swift
// Пример подписки: экран решает, что показывать, по isAuthenticated.
...
```swift
let fresh = try await AuthService.shared.fetchMe()   // пишется в currentUser
```

**4. Выход:**

```swift
AuthService.shared.logout()   // @MainActor — вызывать с главного актора
```
...
```
   Старт приложения
        ...
        ▼
   init → restoreSession()
        │  есть токен в Keychain и профиль в UserDefaults?
        ├─ да ─► setAccessToken + currentUser + isAuthenticated=true + socket.connect
        └─ нет ─► остаёмся разлогинены (показывается экран входа)

   login(phone:displayName:)
        │ POST /auth/login → AuthResponse{ accessToken, user }
        ▼
   saveSession(): Keychain.save("access_token") + network.setAccessToken
                  + UserDefaults["current_user"] + currentUser + isAuthenticated=true
                  + socket.connect(token:)

   logout()  (явно ИЛИ по .networkUnauthorized / 401)
        │
        ▼
   Keychain.delete + UserDefaults.remove + network.clearAccessToken
   + socket.disconnect + currentUser=nil + isAuthenticated=false
```

### Детали реализации, важные для интеграции

- **Синглтон.** Единственный экземпляр `AuthService.shared`, `init` приватный.
  Это гарантирует единое состояние сессии на всё приложение и единственную
  подписку на `.networkUnauthorized`.
- **Потокобезопасность / актор.** Мутации состояния и `logout()` выполняются на
  главном акторе: `login`/`fetchMe` оборачивают запись в `await MainActor.run { }`,
  а `logout()` помечен `@MainActor`. Наблюдатель `401` тоже переключается на
  главный поток (`queue: .main` + `Task { @MainActor … }`). Это безопасно для
  привязанного к UI `@Observable`-состояния.
- **Порядок сохранения сессии.** В `saveSession` токен сначала кладётся в Keychain
  и в транспорт, затем кешируется профиль, поднимается флаг и **последним**
  подключается сокет — к моменту `connect` токен уже валиден.
- **Кеш профиля best-effort.** Кодирование профиля в `UserDefaults`
  (`try? JSONEncoder().encode`) не критично: если оно не удастся, вход всё равно
  состоится, но при следующем запуске `restoreSession` не найдёт профиль и сессия
  восстановлена не будет.
- **`restoreSession` требует и токен, и профиль.** Восстановление происходит,
  только если в Keychain есть токен **и** в `UserDefaults` есть декодируемый
  профиль. Отсутствие любого из них → пользователь считается разлогиненным.

### Точки расширения

- **Refresh-токен.** Сейчас реакция на `401` — только `logout()`. Если появится
  refresh-flow, его координацию (обновить токен, переподключить сокет) логично
  разместить здесь, а сам перехват/повтор запроса — в `NetworkService`.
- **Расширение учётных данных.** Логин принимает `phone` + `displayName`
  (`LoginRequest`). При появлении новых способов входа (OTP, OAuth) их методы —
  тоже сюда, поверх того же механизма `saveSession`.
- **Синхронизация профиля.** `fetchMe()` можно дёргать после старта для актуализации
  `currentUser`, не трогая токен и состояние `isAuthenticated`.
