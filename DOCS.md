...
`AuthService` — это **центр управления сессией пользователя**. Он отвечает за то,
кто сейчас вошёл в приложение, и держит это состояние в двух наблюдаемых
свойствах, на которые опирается весь UI:

- `currentUser: User?` — профиль текущего пользователя (или `nil`, если никто не
  вошёл);
- `isAuthenticated: Bool` — флаг «пользователь авторизован».

Модуль решает четыре задачи и является **дирижёром** для остальных частей
бэкенда — сам он данными почти не владеет, а координирует их между
`NetworkService`, `KeychainService`, `SocketService` и локальным хранилищем:

1. **Вход (login).** Отправляет учётные данные на сервер, получает access-токен и
   профиль, сохраняет сессию.
2. **Хранение и восстановление сессии.** Кладёт токен в Keychain, профиль — в
   `UserDefaults`, и при следующем запуске приложения молча восстанавливает вход
   без повторной авторизации.
3. **Выход (logout).** Полностью очищает сессию во всех местах, где она хранится, и
   рвёт связанные соединения.
4. **Реакция на разавторизацию.** Подписан на нотификацию `.networkUnauthorized`
   от `NetworkService` и при протухшем токене автоматически выполняет `logout()`.
...
```
   ...
   AuthViewModel ──────────────────────────────────────►  │                      │
   (экран входа)                                           │     AuthService      │
                                                           │      (.shared)       │
   ContentView / Views ◄──── currentUser / isAuthenticated │                      │
       (наблюдают @Observable-состояние)                   └───┬───┬───┬───┬───────┘
                                                               │   │   │   │
   ┌───────────────────────────────────────────────────────────┘   │   │   └────────────────┐
   ▼                                    ▼                           ▼                          ▼
 NetworkService                     KeychainService            UserDefaults              SocketService
 setAccessToken / clearAccessToken  save/load/delete            профиль User (JSON)      connect(token:) / disconnect()
 "access_token" ─► все REST-запросы  "access_token"            "current_user"           поднять/разорвать WebSocket
                                                           
                    ▲  NotificationCenter (.networkUnauthorized)  │  при HTTP 401
                    └────────────────────────────────────────────┘  → logout()
```

- **Кто вызывает (потребители):** экран входа через `AuthViewModel` вызывает
  `login(...)`; SwiftUI-слой (`ContentView` и др.) **наблюдает** `currentUser` и
  `isAuthenticated`, чтобы решать, какой экран показать (вход или основной). Это
  фронтенд — здесь он упомянут только как потребитель состояния.
- **`NetworkService`.** После логина/восстановления `AuthService` вызывает
  `network.setAccessToken(_:)`, после чего все REST-запросы уходят с заголовком
  `Authorization`. При выходе — `network.clearAccessToken()`. `AuthService` — по
  сути **единственный владелец токена на уровне логики приложения**.
- **`KeychainService`.** Access-токен персистится в Keychain под ключом
  `"access_token"` (`save`/`load`/`delete`). Это безопасное хранилище токена между
  запусками приложения.
- **`UserDefaults`.** Профиль `User` кэшируется как JSON под ключом
  `"current_user"` — чтобы при рестарте сразу показать данные пользователя, не
  дожидаясь сети.
- **`SocketService`.** Именно `AuthService` **управляет жизненным циклом**
  реального времени: `connect(token:)` при появлении сессии и `disconnect()` при
  выходе. Сам `SocketService` токен не хранит и соединение сам не поднимает.
- **`NetworkService → .networkUnauthorized`.** При HTTP `401` сетевой слой
  публикует нотификацию; `AuthService` на неё подписан в `init` и вызывает
  `logout()`. Так протухший токен централизованно приводит к выходу.
- **Модели:** `AuthResponse` (`{ accessToken, user }`) и `LoginRequest`
  (`{ phone, displayName }`) — контракт эндпоинта `/auth/login`; `User` — профиль.

### Публичный API

```swift
static let shared: AuthService            // единственный экземпляр (init приватный)

private(set) var currentUser: User?       // текущий профиль (только чтение снаружи)
private(set) var isAuthenticated: Bool    // авторизован ли пользователь

// POST /auth/login — вход по телефону и имени. Сохраняет сессию, возвращает профиль.
func login(phone: String, displayName: String) async throws -> User

// GET /auth/me — подтянуть/обновить актуальный профиль текущего пользователя.
func fetchMe() async throws -> User

// Полный выход: очистка Keychain, UserDefaults, токена и разрыв WebSocket.
@MainActor func logout()
```

### Как использовать

**1. Вход (обычно из `AuthViewModel` на экране авторизации):**

```swift
do {
    let user = try await AuthService.shared.login(phone: phone, displayName: displayName)
    // сессия уже сохранена, токен установлен, сокет подключён — просто используем user
} catch {
    errorMessage = error.localizedDescription
}
```

**2. Показ нужного экрана по состоянию авторизации (SwiftUI):**
...
// @Observable сам перерисует экран при изменении isAuthenticated
```

**3. Обновление профиля с сервера:**

```swift
let freshUser = try await AuthService.shared.fetchMe()
```

**4. Выход:**

```swift
AuthService.shared.logout()   // очистит сессию и разорвёт соединение
```

### Детали реализации, важные для интеграции

- **Синглтон и авто-восстановление.** `init` приватный и в конструкторе сразу
  вызывает `restoreSession()` — то есть **сессия восстанавливается автоматически
  при первом обращении** к `AuthService.shared`. Отдельно вызывать восстановление
  не нужно.
- **Восстановление требует и токен, и профиль.** `restoreSession()` восстановит
  вход только если в Keychain есть `"access_token"` **и** в `UserDefaults` есть
  валидный профиль. Если чего-то одного нет — вход считается несостоявшимся
  (`guard … else { return }`), и пользователь увидит экран логина.
- **Мутации состояния — на главном потоке.** Изменения `currentUser`,
  `isAuthenticated` и связанные с ними операции обёрнуты в `MainActor`
  (`saveSession`/`logout` помечены `@MainActor`, а `fetchMe`/`login` пишут в
  состояние через `await MainActor.run { … }`). Это безопасно для UI и Swift 6
  concurrency.
- **`logout()` идемпотентен и всеобъемлющ.** Он чистит **все** места хранения
  сессии сразу: Keychain, `UserDefaults`, токен в `NetworkService` и соединение в
  `SocketService`. Повторный вызов безопасен.
- **Подписка на `.networkUnauthorized`.** Наблюдатель создаётся в `init` на
  очереди `.main` и хранится в `unauthorizedObserver`. Обработчик оборачивает
  вызов в `Task { @MainActor … }`, чтобы уйти на главный актор.
- **Ошибки.** `login`/`fetchMe` пробрасывают `NetworkError` из `NetworkService` —
  вызывающая сторона должна ловить их (см. пример 1).

### Точки расширения

- **Refresh-токен.** Сейчас refresh-flow нет: при `401` пользователь
  разлогинивается. Логику обновления токена удобно встроить в связку
  «`NetworkService` перехватывает 401 → `AuthService` обновляет токен и
  переустанавливает его», не трогая доменные сервисы.
- **Разлогин на нескольких устройствах / отзыв сессии.** Точка для серверного
  вызова `POST /auth/logout` — сейчас выход только локальный.
- **Больше провайдеров входа.** `login(...)` заточен под пару «телефон + имя».
  Новые способы входа (SMS-код, OAuth) добавляются рядом, переиспользуя
  `saveSession(_:)` как общий финализатор сессии.
